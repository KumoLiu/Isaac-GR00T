#!/usr/bin/env bash
# 30k-step finetune with cam_head only, episodes [0, 190), 4 GPUs (DDP).
set -euo pipefail
...
CUDA_VISIBLE_DEVICES=0,1,2,3 NUM_GPUS=4 MASTER_PORT=29504 \
MAX_STEPS=30000 SAVE_STEPS=3000 USE_WANDB=0 \
uv run bash examples/finetune.sh \
  --base-model-path nvidia/GR00T-N1.7-3B \
  --dataset-path /localhome/local-yunl/g1_pick_trocar_200_train0_190 \
  --modality-config-path examples/g1-dex3/g1_dex3_head_config.py \
  --embodiment-tag NEW_EMBODIMENT \
  --use-ddp \
  --output-dir /localhome/local-yunl/Isaac-GR00T/g1_pick_trocar_200_finetune/head_30k
