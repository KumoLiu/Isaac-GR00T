#!/usr/bin/env bash
# 50k-step finetune with cam_head only, episodes [0, 190), 4 GPUs (DDP).
set -euo pipefail
...
CUDA_VISIBLE_DEVICES=0,1,2,3 NUM_GPUS=4 MASTER_PORT=29505 \
...
uv run bash examples/finetune.sh \
  ...
  --use-ddp \
  --output-dir /localhome/local-yunl/Isaac-GR00T/g1_pick_trocar_200_finetune/head_50k
